...
class Split(nn.Module):
    def __init__(self, layers, split_size, dim):
...
class EncoderWrapper(nn.Module):
    def __init__(self, inp_chans, num_layers, hidden_dim, act_layer):
        ...
        input_dims = [i * 4 for i in split_size]
        output_dims = [100,100,100,100,100,100]
        # output_dims = [80,80,80,80,80,80]
        # output_dims = [60,60,60,60,60,60]
        print(output_dims)
        encoders = [Encoder(num_layers, i, o, hidden_dim, act_layer) for i, o in zip(input_dims, output_dims)]
        self.split = Split(encoders, split_size, dim=2)
...
class DecoderWrapper(nn.Module):
    def __init__(self, num_layers, input_dim, hidden_dim, act_layer):
        super(DecoderWrapper, self).__init__()
        output_dims = [8, 13, 13, 13, 13, 13]
        decoders = [EncoderDecoder(num_layers, input_dim, o, hidden_dim, act_layer) for o in output_dims]
        self.parallel = Parallel(decoders)
