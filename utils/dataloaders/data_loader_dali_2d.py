...
class ERA5DaliESDataloader(object):
    def get_pipeline(self):
        ...
        with pipeline:
            ...
            if self.train and self.roll:
                ...
                inp = fn.cat(inp[:, :, :, shift:], inp[:, :, :, :shift], device="cpu", axis=3)

                tar = fn.cat(tar[:, :, :, shift:], tar[:, :, :, :shift], device="cpu", axis=3)

                if self.add_zenith:
                    izen = fn.cat(izen[:, :, :, shift:], izen[:, :, :, :shift], device="cpu", axis=3)

                    tzen = fn.cat(tzen[:, :, :, shift:], tzen[:, :, :, :shift], device="cpu", axis=3)

            # normalize if requested
            if self.normalize:
                inp = fn.normalize(inp, device="cpu", axis_names=self.norm_channels, batch=self.norm_batch, mean=self.in_bias, stddev=self.in_scale)

                tar = fn.normalize(tar, device="cpu", axis_names=self.norm_channels, batch=self.norm_batch, mean=self.out_bias, stddev=self.out_scale)

            # add noise if requested
            if self.add_noise:
                inp = fn.noise.gaussian(inp, device="cpu", stddev=self.noise_std, seed=self.local_seed)
    ...
    def __init__(self, params, location, train, seed=333, final_eval=False):
        ...
        # self.iterator = DALIGenericIterator([self.pipeline], outnames, auto_reset=True, size=-1, last_batch_policy=LastBatchPolicy.DROP, prepare_first_batch=False)
    ...
    def __iter__(self):
        ...
        for token in self.iterator:
            ...
            # convert grid
            # print("data******", result[0].shape, result[1].shape, result[2].shape, result[3].shape)
            with torch.no_grad():
                result = map(lambda x: self.grid_converter(x), result)

            # ### 80 iter
            result = map(lambda x: x.cpu(), result)
            # ### end
            yield result
