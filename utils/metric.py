...
from makani.utils.metrics.functions import GeometricL1, GeometricRMSE, GeometricACC, GeometricACC_CMA, Quadrature
...
class MetricsHandler:
    ...
    def __init__(
        ...
        clim,
        device,
        rmse_var_names=["u10m", "t2m", "u500", "z500", "r500", "q500", "t850"],
        acc_vars_names=["u10m", "t2m", "u500", "z500", "r500", "q500" , "t850"],
        acc_auc_var_names=["u10m", "t2m", "u500", "z500", "r500", "q500", "t850"],
    ):
        self.device = device
        self.log_to_screen = params.log_to_screen
        self.log_to_wandb = params.log_to_wandb
        self.channel_names = params.channel_names
        print('rmse_var_names', rmse_var_names)
        print('acc_vars_names', acc_vars_names)
        print('acc_auc_var_names', acc_auc_var_names)
        # set a stream
        ...
        self.acc_cma_handle = GeometricACC_CMA(
            quadrature_rule_type,
            img_shape=self.img_shape,
            crop_shape=self.crop_shape,
            crop_offset=self.crop_offset,
        ...
        self.acc_cma_handle = torch.compile(self.acc_cma_handle, mode="max-autotune-no-cudagraphs")
    ...
    def initialize_buffers(self):
        ...
        self.count = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        self.mu_x = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        self.mu_y = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        self.s_xx = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        self.s_yy = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        self.s_xy = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device=self.device)
        ...
        self.count_cpu = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
        self.mu_x_cpu = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
        self.mu_y_cpu = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
        self.s_xx_cpu = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
        self.s_yy_cpu = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
        self.s_xy_cpu = torch.zeros((self.out_channels_local, self.valid_autoreg_steps + 1), dtype=torch.float32, device="cpu", pin_memory=pin_memory)
    ...
    def update(self, prediction, target, loss, idt):
        ...
        def ss_iteration(count_old,mean_old,ss_old,count_new,mean_new,ss_new):
            ...
            ss_total= rate1*(ss_old + torch.pow(rate2 *(mean_old - mean_new),2))
            ss_total += rate2 * (ss_new + torch.pow(rate1 * (mean_old - mean_new), 2))
        ...
        def sxy_iteration(count_old,meanx_old,meany_old,sxy_old,count_new,meanx_new,meany_new,sxy_new):
            ...
            # print(sxy_total.shape, rate1.shape, meanx_old.shape, meanx_new.shape, count_old.shape)
        ...
        count, mu_x, mu_y, s_xx, s_yy, s_xy = self.acc_cma_handle(prediction - self.clim, target - self.clim)
        _, _, self.s_xx[:, idt] = ss_iteration(self.count[:, idt], self.mu_x[:, idt], self.s_xx[:, idt],
                                               count,mu_x,s_xx)
        _, _, self.s_yy[:, idt] = ss_iteration(self.count[:, idt], self.mu_y[:, idt], self.s_yy[:, idt],
                                               count,mu_y,s_yy)
        self.count[:, idt], self.mu_x[:, idt], self.mu_y[:, idt], self.s_xy[:, idt] = sxy_iteration(self.count[:, idt], self.mu_x[:, idt], self.mu_y[:, idt],
                                                                                                    self.s_xy[:, idt], count, mu_x.squeeze(0), mu_y.squeeze(0), s_xy.squeeze(0))
        # only update this at the first step
        # print(self.count[:, idt], self.s_xx[:, idt], self.s_yy[:, idt], self.s_xy[:, idt], flush=True)
        if idt == 0:
    ...
    def finalize(self, final_inference=False):
        ...
        with torch.no_grad():
            ...
            with torch.cuda.stream(self.stream):
                ...
                # self.count_cpu.copy_(self.count, non_blocking=True)
                # self.mu_x_cpu.copy_(self.mu_x, non_blocking=True)
                # self.mu_y_cpu.copy_(self.mu_y, non_blocking=True)
                # self.s_xx_cpu.copy_(self.s_xx, non_blocking=True)
                # self.s_yy_cpu.copy_(self.s_yy, non_blocking=True)
                # self.s_xy_cpu.copy_(self.s_xy, non_blocking=True)
        ...
        if final_inference:
            return logs, self.acc_curve, self.rmse_curve, self.count, self.mu_x, self.mu_y, self.s_xx, self.s_yy, self.s_xy
        else:
            return logs
