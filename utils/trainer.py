...
class Trainer:
    ...
    def _update_parameters(self, params):
        ...
        if not hasattr(params, "load_loss"):
            params["load_loss"] = False
            # if weight trainable, turn true
        return params
    ...
    def __init__(self, params, world_rank, job_type="train"):
        ...
        if hasattr(params, "log_to_wandb") and params.log_to_wandb:
            ...
            if not params.resuming:
                ...
                wandb_config = YParams(os.path.join(params.wandb_dir, "wandb", "latest-run", "files", "config.yaml"), "_wandb")
                ...
                self.wandb_run = wandb.init(dir=params.wandb_dir, project=params.wandb_project, id=params["wandb_run_id"], resume=True)
        ...
        all_parameters = list(self.model.parameters()) + list(self.loss_obj.parameters())
        ...
        print('init channel weight', self.loss_obj.channel_weights)
    ...
    def train(self):
        ...
        if self.finetune:
            endEpoch = self.startEpoch + 2
        else:
            endEpoch = self.params.max_epochs
        best_valid_loss = 1.0e6
        for epoch in range(self.startEpoch, endEpoch):
        # for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            if (self.data_parallel_rank == 0) and (self.params.save_checkpoint != "none"):
                print("sigma", self.loss_obj.channel_weights)
                # checkpoint at the end of every epoch
                self.save_checkpoint(self.params.checkpoint_path, checkpoint_mode=self.params["save_checkpoint"], if_save_loss=True)
                ...
                if (not self.params.skip_validation) and ((not best_checkpoint_saved) or (valid_logs["base"]["validation loss"] <= best_valid_loss)):
                    self.save_checkpoint(self.params.best_checkpoint_path, checkpoint_mode=self.params["save_checkpoint"], if_save_loss=True)
    ...
    def save_checkpoint(self, checkpoint_path, model=None, checkpoint_mode="flexible", if_save_loss=False):
        """
        Save out checkpoint
        """
        ...
        with torch.no_grad():
            ...
            if checkpoint_mode == "legacy":
                ...
                if if_save_loss:
                    print("saving loss")
                    store_dict['Loss_weight_state'] = self.loss_obj.state_dict()
    ...
    def restore_checkpoint(self, checkpoint_path, checkpoint_mode="flexible", load_optimizer=False, load_scheduler=False, load_counters=False, load_loss=False):
        """
        ...
        if checkpoint_mode == "legacy":
            ...
            if load_loss:
                print('loading loss')
                self.loss_obj.load_state_dict(checkpoint["Loss_weight_state"])
        # new flexible mode allows to load models in arbitrary model-parallel configurations
            ...
            with torch.inference_mode():
                with torch.no_grad():
                    for k_, v in self.model.named_parameters():
                        if (k:="module."+k_ if k_[:6]!="module" else k_) in state_dict.keys():
                            weight = state_dict[k]
