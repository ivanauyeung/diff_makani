...
class LossHandler(nn.Module):
    ...
    def __init__(self, params):
        ...
        if "weighted" in loss_type:
            ...
            if params.channel_weights == "trainable":
                self.trainable = True
                #graphcast setting
                weight_per_hpa = 1.0 / (1000 + 925 + 850 + 700 + 600 + 500 + 400 + 300 + 250 + 200 + 150 + 100 + 50)
                channel_weights = torch.ones(params.N_out_channels, dtype=torch.float32)
                for c, chn in enumerate(params.channel_names):
                    if chn in ["u10m", "v10m", "u100m", "v100m", "tp", "tcwv"]:
                            channel_weights[c] = 0.1
                    elif chn in ["t2m", "sp"]:
                        channel_weights[c] = 1.0
                    # elif chn in ["z500", "u500", "v500", "t500", "r500", "q500", "t850"]:
                    #     channel_weights[c] = 1.0
                    elif chn[0] in ["z", "u", "v", "t", "r", "q"]:
                        pressure_level = float(chn[1:])
                        channel_weights[c] = weight_per_hpa * pressure_level
                        # channel_weights[c] = 0.001 * pressure_level
                    else:
                        channel_weights[c] = 0.1
                
                sigma = torch.sqrt(1.0 / 2 / channel_weights)
                self.channel_weights = nn.Parameter(torch.log(sigma))
            elif params.channel_weights == "auto":
                channel_weights = torch.ones(params.N_out_channels, dtype=torch.float32)
                weight_per_hpa = 1.0 / (1000 + 925 + 850 + 700 + 600 + 500 + 400 + 300 + 250 + 200 + 150 + 100 + 50)
                for c, chn in enumerate(params.channel_names):
                    if chn in ["u10m", "v10m", "u100m", "v100m", "tp", "tcwv"]:
                        channel_weights[c] = 0.1
                    elif chn in ["t2m", "sp", "z500"]:
                        channel_weights[c] = 1.0
                    # elif chn in ["z500", "u500", "v500", "t500", "r500", "q500", "t850"]:
                    #     channel_weights[c] = 1.0
                    elif chn[0] in ["z", "u", "v", "t", "r", "q"]:
    ...
    def forward(self, prd: torch.Tensor, tar: torch.Tensor, inp: torch.Tensor):
        if not self.trainable:
            ...
            # self.channel_weights = log sigma
            chw = 1 / 2 / torch.exp(self.channel_weights) ** 2

            if self.training:
                chw = (chw * self.multistep_weight).reshape(1, -1)
            else:
                chw = chw.reshape(1, -1)
            return self.loss_obj(prd, tar, chw) + torch.sum(self.channel_weights)
