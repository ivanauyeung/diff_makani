...
class GeometricACC_CMA(torch.nn.Module):
    def __init__(self, grid_type, img_shape, crop_shape=None, crop_offset=(0, 0), normalize=False, channel_reduction="mean", batch_reduction="mean", eps=1e-8):
        super(GeometricACC_CMA, self).__init__()
    ...
    def forward(self, x, y):
        mu_x = self.quadrature(x)
        mu_y = self.quadrature(x)
        count = self.quadrature.quad_weight.sum()

        d_x = x - mu_x.unsqueeze(-1).unsqueeze(-1)
        d_y = y - mu_y.unsqueeze(-1).unsqueeze(-1)

        s_xx = self.quadrature(torch.square(d_x))
        s_yy = self.quadrature(torch.square(d_y))
        s_xy = self.quadrature(d_x * d_y)
        ...
        return count, mu_x, mu_y, s_xx, s_yy, s_xy
